window.i18nData = window.i18nData || {};
window.i18nData.en = window.i18nData.en || {};
window.i18nData.en.k = {
    "alchemy": {
        "desc": {
            "keira": "Keira will only appear if you told her to go to Kaer Morhen at the end of the '__quest__' quest. "
        },
        "popuptitle": {
            "keira": "Keira Metz"
        }
    },
    "armourer": {
        "desc": {
            "zoltan": "Zoltan will only appear if he was spoken to during the '__quest__' quest. "
        },
        "popuptitle": {
            "zoltan": "Zoltan Chivay"
        }
    },
    "poi": {
        "desc": {
            "leoGrave": "Grave of the witcher Leo from the first Witcher game. ",
            "deadDog": "Dead dog that shows up in Kaer Morhen only after sleeping with Yennefer during the '__mquest__' mission. To be able to sleep with her, you have to finish the '__squest__' mission and stay in a romantic relationship with her. ",
            "graveyard": " "
        },
        "label": {
            "leoGrave": "Leo's Grave",
            "deadDog": "Dead Dog",
            "graveyard": "Kaer Morhen Graveyard"
        }
    },
    "signpost": {
        "desc": {
            "bastion": "Once young witchers trained here. Now the bastion is a forgotten ruin. ",
            "ironMine": "Iron ore was once extracted from this place for use in the Wolf School's forges. ",
            "kaerMorhen": "The keep of the Sea of Yore, home to the witchers of the Wolf School. ",
            "lakesideHut": "A dozen years ago a hermit lived here who brewed excellent mead. The witchers would visit him from time to time and chat about far-off lands. ",
            "ruinedWatchtower": "This lakeside watchtower was once the pride of the Wolf School witchers, yet today lies in ruins, the only sound the wind whistling past its stones. "
        },
        "label": {
            "bastion": "Bastion",
            "ironMine": "Iron Mine",
            "kaerMorhen": "Kaer Morhen",
            "lakesideHut": "Lakeside Hut",
            "ruinedWatchtower": "Ruined Watchtower"
        }
    }
};
